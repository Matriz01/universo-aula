...
- [x] 4.1 (TEST) `tests/unit/scenes/usePlanetPosition.test.ts` — crear helper `computeAt(planet, level, tDays)` que simula `elapsed` sin React. Assertions: Earth nivel Explorador t=0 tiene Y=0 y módulo (x,z) ≈ visualDistance(1.0); Mars nivel Explorador t=orbital\*period completa la vuelta; Mercury nivel Aprendiz en t=period/4 satisface ecuación de elipse; Mercury nivel Investigador con inclinación produce Y≠0; `solveKeplerNewtonRaphson` ya testeada en 1.9.
- [x] 4.2 (IMPL) `src/scenes/hooks/usePlanetPosition.ts` — implementar los tres modelos del design §4.2 usando `useFrame` + `useRef`; memoizar derivados con `useMemo`; importar `solveKeplerNewtonRaphson`, `applyOrbitalRotation`, `visualDistance` de sus módulos.
- [x] 4.3 (TEST) `tests/unit/scenes/useOrbitPath.test.ts` — nivel Explorador Earth: 128 puntos, todos con Y=0, todos con módulo ≈ visualDistance(1.0); nivel Aprendiz Mercury: puntos forman elipse (max/min radio difieren); el primer y último punto son iguales.
- [x] 4.4 (IMPL) `src/scenes/hooks/useOrbitPath.ts` — genera array de `THREE.Vector3` para un periodo completo; segmentos configurables (default 128); respeta el modelo del nivel activo.
- [x] 4.5 (TEST) `tests/unit/scenes/Planet.test.tsx` — mock-render `<Planet planet={earthData} level="aprendiz" />`: assertar que existe un mesh con textura asignada; assertar que en `variant="dwarf"` el componente monta sin errores.
- [x] 4.6 (IMPL) `src/scenes/components/Planet.tsx` — `SphereGeometry(visualRadius(planet.radius_km), segments)`, `MeshStandardMaterial` con `useTexture` (Drei); `<Html>` de Drei para label; Suspense fallback con color sólido; `onClick` → callback.
- [x] 4.7 (IMPL) `src/scenes/components/OrbitPath.tsx` — `<Line>` de Drei con los puntos de `useOrbitPath`; visible/invisible según prop `visible`; color tenue y opacidad 0.3.
- [x] 4.8 (IMPL) `src/scenes/components/PlanetMoon.tsx` — órbita simplificada alrededor de la Tierra (radio ~0.8 unidades, periodo 27.3 días); `SphereGeometry(0.27, 16, 16)` + textura `moon/2k.jpg`.
- [x] 4.9 (IMPL) `src/scenes/components/Saturn.tsx` — `<RingGeometry>` con `innerRadius` y `outerRadius` de `planet.rings`; material con `alphaMap` de `saturn-rings/2k.png`; `side={THREE.DoubleSide}`; inclinación axial.
- [x] 4.10 (TEST) `tests/unit/scenes/AsteroidBelt.test.tsx` — mock `useGpuCapability` → `'high'`/`'low'`/`'mid'`; assertar que `IcosahedronGeometry` se crea con `detail=0`; montaje sin errores para cada GPU tier.
- [x] 4.11 (IMPL) `src/scenes/components/AsteroidBelt.tsx` — `<instancedMesh>` con `IcosahedronGeometry(0.02, 0)`, `MeshStandardMaterial`; distribución log-normal en radio, gaussiana en Y; rotaciones aleatorias por instancia (semilla Mulberry32); count adaptado a `useGpuCapability()`.
- [x] 4.12 (REFACTOR) Semilla determinista exportable (`0xdeadbeef`); `instancedMesh` tiene `name="asteroid-belt"` asignado en `useEffect`. Hook `usePlanetsData` para carga lazy de `planets.json`.
